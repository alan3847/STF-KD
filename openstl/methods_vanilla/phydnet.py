...
class PhyDNet(Base_method):
    ...
    def compute_loss(self, batch, **kwargs):
        ...
        # When nested inside KDMethod, use synced outer epoch
        current_epoch = getattr(self, '_kd_current_epoch', self.current_epoch)
        teacher_forcing_ratio = np.maximum(0, 1 - current_epoch * 0.003)

        # native PhyDNet training loss
        native_loss = self.model(batch_x, batch_y, self.constraints, teacher_forcing_ratio)

        # prediction still uses native inference path
